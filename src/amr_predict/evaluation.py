...
def plot_activation_density(activations: Tensor, latent_idx: int | Sequence[int]):
    """Plot latents' activation density
    ...
    frac_active = (selected > 0).sum(dim=0) / selected.shape[0] * 100
    info = [
        f"  Latent {i}: {round(frac, 2)}%"
        for i, frac in enumerate(frac_active.tolist())
    ]
    to_df = pl.DataFrame(selected).rename(lambda x: x.removeprefix("column_"))
    long = to_df.unpivot(variable_name="latent", value_name="Activation")
    title = f"Activation density\n{'\n'.join(info)}"
    return (
        gg.ggplot(long, gg.aes(x="Activation", fill="latent"))
        + gg.geom_histogram(position="dodge", binwidth=0.1)
        + gg.ggtitle(title)
    )
