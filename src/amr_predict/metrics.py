    ...
    def _get_category_matrix(
        ...
        category_mat: Tensor = torch.hstack(
            [
                ...
                torch.vstack([torch.tensor(cats[n]) for n in neighbors[indices, :]]),
            ]
        )
...
class PerturbationMetrics:
        ...
        validator=validators.or_(
            ...
            validators.deep_mapping(
                key_validator=validators.in_(
                    ["natural", "perturbed", "random", "shared"],
                ),
    ...
    # Autogenerated dataframe aligning entries between datasets based on
    # `id_col`. Columns are named for each dataset type: `natural`, `perturbed`
    # and values are entry indices
    ...
    def _distance_correlation(self) -> pl.DataFrame:
        ...
        take_indices: None | np.array = None
        n: int = len(self.natural)
        idx1 = self.idx_df["natural"]
        if self.subsample_prop and (
            frac_subsample := self.subsample_prop.get("shared")
        ):
            take_indices = self.rng.choice(range(n), size=int(n * frac_subsample))
            idx1 = idx1[take_indices]
        for pair_name, to_compare in zip(
            ...
            idx2 = (
                ...
                else self.rng.choice(range(n), size=len(idx1))
            )
            if take_indices is not None:
                idx2 = idx2[take_indices]
    ...
    def _measure_classifiability(self):
        ...
        """
        result = {"group": [], "mcc": []}
        subsampled = self._maybe_subsample()
        natural, perturbed, random = (
            subsampled["natural"],
            subsampled["perturbed"],
            subsampled["random"],
        )
        for group, dset_pair in zip(
            ...
            if group == "cv_natural_w_random":
                x_test = perturbed[perturbed.x_key][:]
                len_test = len(x_test)
    ...
    def _maybe_subsample(self) -> dict:
        result = {}
        for k in ("natural", "perturbed", "random"):
            dset: LinkedDataset | None = getattr(self, k)
            subsample_prop = self.subsample_prop.get(k) if self.subsample_prop else None
            if subsample_prop:
                result[k] = dset.sample(fraction=subsample_prop, seed=self.seed)
            else:
                result[k] = dset
        return result
    ...
    def _random_neighbor_score(self) -> dict:
        ...
        return {
            "natural": random_neighbor_score(
                natural=subsampled["natural"],
                random=subsampled["random"],
                **self.rns_kws,
            ),
            "perturbed": random_neighbor_score(
                natural=subsampled["perturbed"],
                random=subsampled["random"],
                **self.rns_kws,
            ),
        }
